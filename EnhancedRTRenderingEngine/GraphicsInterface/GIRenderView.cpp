...
void GIRenderView::Initialize(GIImmediateCommands* cmd, const ViewportParam& param) {
    ...
    _OMResource->depthStencilView = GICommandUtils::CreateDepthStencilView(cmd, Size2Dd(param.cfg.width, param.cfg.height), TextureFormat::D24_UNORM_S8_UINT, false);
    ...
    _renderSize = Size2Dd(static_cast<unsigned int>(param.cfg.width), static_cast<unsigned int>(param.cfg.height));
